Feature: Search User Product
  ...
  Scenario: Get Search user product with valid token and valid parameter product name
    Given GET Search product with "valid" token and "valid" parameter product name "Guitar"
    When Send GET Search product
    Then Status code should be 200 OK
    And Response body should display search user product name "Guitar"
  ...
  Scenario: Get Search user product with invalid token and valid parameter product name
    Given GET Search product with "invalid" token and "valid" parameter product name "Guitar"
    When Send GET Search product
